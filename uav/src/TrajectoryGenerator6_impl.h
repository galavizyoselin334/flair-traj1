...
public:
  ...
  flair::gui::DoubleSpinBox *Xf;          // (ojo) no se crea en tu ctor actual
  flair::gui::DoubleSpinBox *Yf;          // (ojo) no se crea en tu ctor actual

private:
  // métodos para la generación de trayectorias polinomiales
  ...
  // orientación
  float computed_yaw;   // yaw deseado
  bool yaw_frozen;      // congelar yaw después de tfac
  float frozen_yaw;     // yaw congelado
  float targetYaw;      // yaw del target desde VRPN
  float yaw0;           // yaw inicial UAV
};
